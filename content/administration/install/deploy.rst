...
The **multi-threaded** server is a simpler server primarely used for
development, demonstrations and for compatibility with various operation
systems (including Windows). A new thread is spawned for every new HTTP
request, even for long running requests such as longpolling. Extra daemonic
...
The **multi-processing** server is a full blown server primarely used for
production. It is not liable to the same Python limitation (GIL) on resource
usage hence makes best use of the hardware. A pool of workers is created upon
...
configurable process reaper monitores resource usage and is able to
...
In multi-processing, a dedicated LiveChat worker is automatically started and listens on the :option:`--longpolling-port <odoo-bin --longpolling-port>`. By default the HTTP requests will keep on accessing the normal HTTP workers instead of the LiveChat one, you must deploy a proxy in front of Odoo and redirect incomming requests whose path starts with ``/longpolling`` to the LiveChat worker. You must also start Odoo in :option:`--proxy-mode <odoo-bin --proxy-mode>` so it uses the real client headers (such as hostname, scheme and ip) instead of the proxy ones.
...
On Linux like systems, it is recommended to use the multi-processing server over
the multi-threading one to benefit from the better hardware usage and increased
...
It is possible to reuse the Odoo cron server to serve the live chat requests.
